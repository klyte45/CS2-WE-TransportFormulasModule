...
public partial class WE_TFM_PlatformMappingSystem : BelzontBasicSystem
{
    ...
    protected override void OnCreateWithBarrier()
    {
        m_connectableRoutesNotMapped = GetEntityQuery(new EntityQueryDesc[] {
                new() {
                    ...
                    None =
                    [
                        ComponentType.ReadOnly<WE_TFM_PlatformData>(),
        ...
        m_connectionsUpdatedPlatforms = GetEntityQuery(new EntityQueryDesc[] {
              new() {
                    All =
                    [
                        ComponentType.ReadOnly<WE_TFM_PlatformConnectionsUpdated>(),
        ...
        m_resetWaypointsQuery = GetEntityQuery([
            new EntityQueryDesc
            {
                All = [ComponentType.ReadOnly<WE_TFM_WaypointDestinationConnections>()]
            }
        ]);
    }
    ...
    protected override void OnUpdate()
    {
        ...
        if (shallReset)
        {
            Barrier.CreateCommandBuffer().RemoveComponent<WE_TFM_WaypointDestinationConnections>(m_resetWaypointsQuery, EntityQueryCaptureMode.AtPlayback);
            shallReset = false;
        }
        if (!m_connectableRoutesNotMapped.IsEmptyIgnoreFilter)
        {
            ...
            foreach (var kvp in valuesToSet)
            {
                ...
                DynamicBuffer<WE_TFM_PlatformMappingLink> buffer;
                if (EntityManager.TryGetBuffer<WE_TFM_PlatformMappingLink>(kvp.Key, true, out var currentBuffer))
                {
                    for (int i = 0; i < currentBuffer.Length; i++)
                    {
                        valuesToAdd.Add(currentBuffer[i]);
                    }
                    buffer = cmdBuffer.SetBuffer<WE_TFM_PlatformMappingLink>(kvp.Key);
                    buffer.Clear();
                }
                else
                {
                    buffer = cmdBuffer.AddBuffer<WE_TFM_PlatformMappingLink>(kvp.Key);
                }
                ...
                foreach (var route in sortedValues)
                {
                    ...
                    var data = new WE_TFM_PlatformData
                    {
                        type = route.Item3,
                    ...
                    cmdBuffer.AddComponent<WE_TFM_PlatformConnectionsUpdated>(route.routePlatformData.platformData);
                }
        ...
        if (!m_connectionsUpdatedPlatforms.IsEmpty)
        {
            new ConnectionsSortingJob
            {
                ...
                m_waypointDestinationsLookup = GetBufferLookup<WE_TFM_WaypointDestinationConnections>(true),
    ...
    private struct ConnectionsSortingJob : IJobChunk
    {
        ...
        public BufferLookup<WE_TFM_WaypointDestinationConnections> m_waypointDestinationsLookup;
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            try
            {
                for (int i = 0; i < entities.Length; i++)
                {
                    ...
                    cmdBuffer.RemoveComponent<WE_TFM_PlatformConnectionsUpdated>(unfilteredChunkIndex, entity);
                    if (m_connectedRouteLookup.TryGetBuffer(entity, out var connectedRoutes))
                    {
                        ...
                        sortedData.Sort();
                        cmdBuffer.RemoveComponent<WE_TFM_WaypointDestinationConnections>(unfilteredChunkIndex, entity);
                        var results = cmdBuffer.AddBuffer<WE_TFM_WaypointDestinationConnections>(unfilteredChunkIndex, entity);
                        results.Clear();
        ...
        private readonly struct WaypointDestinationSortable(WE_TFM_WaypointDestinationConnections connection, int routeNumber)
            : IComparable<WaypointDestinationSortable>, IComparer<WaypointDestinationSortable>, IEquatable<WaypointDestinationSortable>
        {
            public readonly WE_TFM_WaypointDestinationConnections connection = connection;
    ...
    private readonly struct PairEntityRoute(Entity target, WE_TFM_PlatformMappingLink route) : IEquatable<PairEntityRoute>
    {
        public readonly Entity target = target;
        public readonly WE_TFM_PlatformMappingLink route = route;
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            for (int i = 0; i < entities.Length; i++)
            {
                ...
                if (m_ownerLookup.TryGetComponent(entities[i], out var owner) && owner.m_Owner != Entity.Null)
                {
                    ...
                    m_hashSet.Add(new(owner.m_Owner, new WE_TFM_PlatformMappingLink(entities[i], ownTransform.m_Position, parentMatrix.MultiplyPoint(ownTransform.m_Position), isBasePlatform)));
                }
                if (m_attachedLookup.TryGetComponent(entities[i], out Attached parent) && parent.m_Parent != Entity.Null)
                {
                    ...
                    var parentMatrix = Matrix4x4.TRS(parentTransform.m_Position, parentTransform.m_Rotation, Vector3.one).inverse;
                    m_hashSet.Add(new(target, new WE_TFM_PlatformMappingLink(entities[i], ownTransform.m_Position, parentMatrix.MultiplyPoint(ownTransform.m_Position), isBasePlatform)));
                }
            }
