...
{
    public partial class WE_TFM_ApplyRouteWatchSystem : BelzontBasicSystem
    {
        ...
        private struct MarkDirtyConnectionsOnToolApplyJob : IJobChunk
        {
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                ...
                for (int i = 0; i < nativeArray.Length; i++)
                {
                    ...
                    if (m_TempType.TryGetComponent(entity, out var temp))
                    {
                        ...
                        {
                            m_cmdBuffer.RemoveComponent<WE_TFM_VehicleIncomingOrderData>(unfilteredChunkIndex, connected.m_Connected);
                        }
                        if (m_WaypointConnectionData.TryGetComponent(temp.m_Original, out var connected2) && m_WaypointIncomingData.HasComponent(connected2.m_Connected))
                        {
                            ...
                            m_cmdBuffer.RemoveComponent<WE_TFM_VehicleIncomingOrderData>(unfilteredChunkIndex, connected2.m_Connected);
                        }
                        m_cmdBuffer.AddComponent<WE_TFM_WaypointDestinationConnectionsDirtyPre>(unfilteredChunkIndex, entity);
                        m_cmdBuffer.AddComponent<WE_TFM_WaypointDestinationConnectionsDirtyPre>(unfilteredChunkIndex, temp.m_Original);
                    }
                }
            }
            ...
            [ReadOnly] public ComponentLookup<WE_TFM_VehicleIncomingOrderData> m_WaypointIncomingData;
        }
    }
}