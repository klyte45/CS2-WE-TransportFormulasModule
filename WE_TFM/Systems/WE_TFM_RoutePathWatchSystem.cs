...
{
    public partial class WE_TFM_RoutePathWatchSystem : BelzontBasicSystem
    {
        ...
        private struct RoutePathReadyJob : IJobChunk
        {
            ...
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                ...
                for (int i = 0; i < entities.Length; i++)
                {
                    ...
                    {
                        m_cmdBuffer.AddComponent<WE_TFM_DirtyTransportLine>(unfilteredChunkIndex, owner.m_Owner);
                        if (m_connectedLookup.TryGetComponent(pathUpdated.m_Owner, out var connected))
                        {
                            m_cmdBuffer.AddComponent<WE_TFM_WaypointDestinationConnectionsDirtyPre>(unfilteredChunkIndex, pathUpdated.m_Owner);
                        ...
                        && m_routeLookup.HasComponent(owner.m_Owner))
                    {
                        ...
                        {
                            m_cmdBuffer.AddComponent<WE_TFM_WaypointDestinationConnectionsDirtyPre>(unfilteredChunkIndex, pathMoved.m_Target);
                            m_cmdBuffer.RemoveComponent<WE_TFM_VehicleIncomingOrderData>(unfilteredChunkIndex, connected.m_Connected);
                        }
                    }
                }
