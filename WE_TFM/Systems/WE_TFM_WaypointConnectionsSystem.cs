...
{
    public partial class WE_TFM_WaypointConnectionsSystem : BelzontBasicSystem, IDefaultSerializable
    {
        ...
        private static WE_TFM_WaypointConnectionsSystem Instance;
        ...
        public void Deserialize<TReader>(TReader reader) where TReader : IReader
        {
            ...
            {
                throw new System.Exception($"Unsupported version {version} for {nameof(WE_TFM_WaypointConnectionsSystem)}.");
            }
        ...
        {            
            ...
            m_unmappedOrPreDirtyStops = GetEntityQuery(new EntityQueryDesc[]
            {
                new()
                {
                    ...
                    None = [
                        ComponentType.ReadOnly<WE_TFM_WaypointDestinationConnections>(),
                        ComponentType.ReadOnly<WE_TFM_WaypointDestinationConnectionsToBeUpdated>(),
                ...
                new()
                {
                    All = [
                        ComponentType.ReadOnly<Waypoint>(),
                        ComponentType.ReadOnly<Connected>(),
                        ComponentType.ReadOnly<WE_TFM_WaypointDestinationConnectionsDirtyPre>()
                    ],
                    None = [
                        ComponentType.ReadOnly<Temp>(),
                        ComponentType.ReadOnly<Deleted>(),
        ...
        protected override void OnUpdate()
        {
            ...
            if (!m_dirtyStops.IsEmpty)
            {
                new DirtyRouteWaypointUpdate
                {
                    ...
                    m_dirtyLookup = GetComponentLookup<WE_TFM_WaypointDestinationConnectionsToBeUpdated>(true),
                    m_attachedLookup = GetComponentLookup<Attached>(true),
                    m_waypointLookup = GetComponentLookup<Waypoint>(true),
                    m_platformsLinkLookup = GetBufferLookup<WE_TFM_PlatformMappingLink>(true),
                    m_ConnectionsLookup = GetBufferLookup<WE_TFM_WaypointDestinationConnections>(true),
                    m_statusesLookup = GetComponentLookup<WE_TFM_LineStatus>(true)
            ...
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                ...
                for (int i = 0; i < pathUpdatedArray.Length; i++)
                {
                    ...
                    if (m_OwnerLookup.TryGetComponent(pathUpdated.m_Owner, out var owner) && m_routeWaypointLookup.TryGetBuffer(owner.m_Owner, out var waypoints))
                    {
                        ...
                        {
                            m_cmdBuffer.RemoveComponent<WE_TFM_WaypointDestinationConnectionsToBeUpdated>(unfilteredChunkIndex, waypoints[j].m_Waypoint);
                            m_cmdBuffer.AddComponent(unfilteredChunkIndex, waypoints[j].m_Waypoint, new WE_TFM_WaypointDestinationConnectionsToBeUpdated
                            {
        ...
        private struct RouteConnectionModifiedJob : IJobChunk
        {
            ...
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                ...
                for (int i = 0; i < entityArray.Length; i++)
                {
                    ...
                    m_cmdBuffer.RemoveComponent<WE_TFM_WaypointDestinationConnectionsDirtyPre>(unfilteredChunkIndex, entity);
                }
            }
        }
//Can't be bursted: Harmony patching needed to work
        private struct UnmappedRouteWaypointsJob : IJobChunk
        {
            ...
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                ...
                {
                    var entity = entityArray[i];
                    m_cmdBuffer.RemoveComponent<WE_TFM_WaypointDestinationConnectionsToBeUpdated>(unfilteredChunkIndex, entity);
                    m_cmdBuffer.AddComponent(unfilteredChunkIndex, entity, new WE_TFM_WaypointDestinationConnectionsToBeUpdated
                    {
                        requestFrame = frameRequest,
        ...
        private struct DirtyRouteWaypointUpdate : IJobChunk
        {
            ...
            public ComponentLookup<WE_TFM_WaypointDestinationConnectionsToBeUpdated> m_dirtyLookup;
            public BufferLookup<WE_TFM_WaypointDestinationConnections> m_ConnectionsLookup;
            public BufferLookup<WE_TFM_PlatformMappingLink> m_platformsLinkLookup;
            public ComponentLookup<WE_TFM_LineStatus> m_statusesLookup;

            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                var entityArray = chunk.GetNativeArray(entityTypeHandle);
                NativeHashMap<Entity, WE_TFM_WaypointDestinationConnections> mappedLines = new(25, Allocator.Temp);
                try
                {
                    for (int i = 0; i < entityArray.Length;)
                    {
                        ...
                        {
                            m_cmdBuffer.RemoveComponent<WE_TFM_WaypointDestinationConnectionsToBeUpdated>(unfilteredChunkIndex, entity);
                            m_cmdBuffer.AddComponent<WE_TFM_WaypointDestinationConnectionsDirtyPre>(unfilteredChunkIndex, entity);
                            ...
                            m_routeWaypointLookup.TryGetBuffer(owner.m_Owner, out var waypoints))
                        {
                            ...
                            for (int j = start; j <= end; j++)
                            {
                                ...
                                if (m_connectedLookup.TryGetComponent(waypoint, out var connectedPlatform))
                                {
                                    ...
                                    {
                                        var buffer = m_cmdBuffer.AddBuffer<WE_TFM_WaypointDestinationConnections>(unfilteredChunkIndex, entity);
                                        buffer.Clear();
                                        m_cmdBuffer.RemoveComponent<WE_TFM_WaypointDestinationConnectionsToBeUpdated>(unfilteredChunkIndex, entity);
                                        goto continueMainLoop;
                                    }
                                    ...
                                    if (m_platformsLinkLookup.TryGetBuffer(buildingEntity, out var platformLinks))
                                    {
                                        for (int l = 0; l < platformLinks.Length; l++)
                                        {
                                            ...
                                            m_cmdBuffer.AddComponent<WE_TFM_PlatformConnectionsUpdated>(unfilteredChunkIndex, refPlatform);
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        m_cmdBuffer.AddComponent<WE_TFM_PlatformConnectionsUpdated>(unfilteredChunkIndex, buildingEntity);
                                    }
                                }
                            }
                            {
            ...
            private bool IterateConnectedRoutes(ref NativeHashMap<Entity, WE_TFM_WaypointDestinationConnections> mappedLines, ref ComponentLookup<WE_TFM_LineStatus> statusLookup, Entity lineOwner, Entity refPlatform)
            {
